...
function VideoModal({ embedUrl, title, onClose }: { embedUrl: string; title: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ backgroundColor: 'rgba(0,0,0,0.97)' }}>
      <div className="flex items-center justify-between px-6 py-5 flex-shrink-0" style={{ background: 'linear-gradient(to bottom, rgba(0,0,0,0.85) 0%, transparent 100%)' }}>
      ...
      <div className="flex-1 flex items-center justify-center px-4 pb-8">
        <div className="w-full max-w-6xl" style={{ aspectRatio: '16/9' }}>
          <iframe src={embedUrl} className="w-full h-full rounded-xl" allow="autoplay; fullscreen; picture-in-picture" allowFullScreen />
        </div>
      </div>
    ...
    <>
      <div className="flex h-screen bg-[#0A0A0B] overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <TopBar user={{ name: 'Admin', role: 'admin' }} title={`Módulo ${modNum} — ${title}`} />
            ...
            <div className="flex h-full">
              <div className="flex-1 flex flex-col overflow-y-auto">
                ...
                <div className="flex items-center gap-2 px-6 pt-5 pb-3 text-xs text-[#52525B]">
                  <a href="/admin/escola" className="hover:text-[#A1A1AA] transition-colors">Escola de Negócios</a>
                ...
                <div className="mx-6 mb-5">
                  <div className="relative bg-[#0A0A0B] rounded-2xl overflow-hidden border border-[#1C1C1E] group" style={{ aspectRatio: '16/9' }}>
                    {loading ? (
                      ...
                      <button onClick={() => setShowPlayer(true)} className="absolute inset-0 w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-[#111113] to-[#0A0A0B] hover:from-[#18181A] transition-all duration-300">
                        ...
                        <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/10 border border-white/20 text-white text-sm font-semibold backdrop-blur-sm">
                          <Maximize className="w-4 h-4" /> Assistir em tela cheia
                        </div>
                      </button>
                    ) : (
                ...
                <div className="px-6 pb-4">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div>
                      <p className="text-[10px] font-semibold text-[#52525B] uppercase tracking-wider mb-1">Módulo {modNum} · Aula {String(aulaNum).padStart(2, '0')}</p>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {hasVideo && (
                      <button onClick={() => setShowPlayer(true)} className="flex items-center gap-1.5 text-xs text-[#7B3FE4] bg-[#7B3FE4]/10 border border-[#7B3FE4]/30 px-3 py-2 rounded-xl hover:bg-[#7B3FE4]/20 transition-colors font-semibold">
                ...
                <div className="px-6 pb-6 flex items-center justify-between gap-3">
                  {prev ? (
                    <a href={`/admin/escola/${modNum}/${prev.num}`} className="flex items-center gap-2 text-sm text-[#71717A] bg-[#111113] border border-[#1C1C1E] px-4 py-2.5 rounded-xl hover:text-white hover:border-[#27272A] transition-colors">
                      <ChevronLeft className="w-4 h-4" /><span className="hidden md:inline">Aula anterior</span>
                    </a>
                  ) : <div />}

                  <a href="/admin/escola" className="flex items-center gap-1.5 text-xs text-[#7B3FE4] hover:text-[#9558EE] transition-colors">
                    <List className="w-3.5 h-3.5" /> Ver todas as aulas
                  </a>

                  {next ? (
                    <a href={`/admin/escola/${modNum}/${next.num}`} className="flex items-center gap-2 text-sm font-semibold text-white bg-gradient-to-r from-[#7B3FE4] to-[#3B82F6] px-4 py-2.5 rounded-xl hover:opacity-90 transition-opacity shadow-[0_0_10px_rgba(123,63,228,0.2)]">
                      <span className="hidden md:inline">Próxima aula</span><ChevronRight className="w-4 h-4" />
                    </a>
                  ) : (
                    <a href="/admin/escola" className="flex items-center gap-2 text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-emerald-600 px-4 py-2.5 rounded-xl hover:opacity-90 transition-opacity">
