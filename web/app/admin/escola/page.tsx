...
const modules = [
  {
    num: 1, slug: 'vendas', title: 'Vendas', subtitle: 'Domine o processo consultivo high ticket',
    color: 'from-[#7B3FE4] to-[#4C1B9B]', border: 'border-[#7B3FE4]/30', icon: TrendingUp, unlocked: true,
    lessons: [
  ...
  {
    num: 2, slug: 'influencia', title: 'Influência', subtitle: 'Construa autoridade e audiência médica',
    color: 'from-[#3B82F6] to-[#1D4ED8]', border: 'border-[#3B82F6]/30', icon: Users, unlocked: true,
    lessons: [
  ...
  {
    num: 3, slug: 'lideranca', title: 'Liderança & Recrutamento', subtitle: 'Forme e lidere times de alto desempenho',
    color: 'from-[#F59E0B] to-[#D97706]', border: 'border-amber-500/30', icon: Zap, unlocked: false,
    lessons: [
  ...
  {
    num: 4, slug: 'modelos', title: 'Modelos de Negócio', subtitle: 'Estruture sua empresa para escala e lucro',
    color: 'from-[#10B981] to-[#059669]', border: 'border-emerald-500/30', icon: Shield, unlocked: false,
    lessons: [
  ...
  return (
    <div className="flex h-screen bg-[#0A0A0B] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <TopBar user={{ name: 'Admin', role: 'admin' }} title="Escola de Negócios" />
        <main className="flex-1 overflow-y-auto px-6 py-5">

          {/* Preview notice */}
          <div className="flex items-center gap-2 bg-[#7B3FE4]/10 border border-[#7B3FE4]/25 rounded-xl px-4 py-2.5 mb-5">
            <span className="text-[10px] font-bold text-[#7B3FE4] uppercase tracking-wider">Preview Admin</span>
            <span className="text-xs text-[#71717A]">Esta é a visão exata que o médico mentorado vê. Para gerenciar conteúdo: <a href="/admin/treinamento" className="text-[#7B3FE4] hover:underline">Gerenciar Treinamento →</a></span>
          </div>
          ...
          <div className="relative bg-[#111113] border border-amber-500/20 rounded-2xl p-5 mb-5 overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div>
                ...
                <h2 className="text-xl font-bold text-white">4 Módulos · 19 Aulas · ~19h de conteúdo</h2>
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => setExpanded(isOpen ? null : mod.num)} className="flex items-center gap-1 text-xs text-[#71717A] hover:text-white transition-colors">
                          Aulas <ChevronDown className={`w-3.5 h-3.5 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
                        </button>
                        {mod.unlocked ? (
                          <a href={`/medical/escola/${mod.slug}/1`} className={`flex items-center gap-1.5 bg-gradient-to-r ${mod.color} text-white text-xs font-semibold px-3 py-1.5 rounded-lg hover:opacity-90 transition-opacity`}>
                            <Play className="w-3 h-3" />{modDone > 0 ? 'Continuar' : 'Iniciar'}
                          </a>
                        ) : (
                          <span className="flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs font-semibold px-3 py-1.5 rounded-lg opacity-60">
                  ...
                  {isOpen && (
                    <div className="border-t border-[#1C1C1E]">
                      {mod.lessons.map((lesson) => (
                        <div key={lesson.num} className={`flex items-center gap-3 px-5 py-3 border-b border-[#1C1C1E] last:border-0 ${mod.unlocked ? 'hover:bg-[#18181A]/60' : 'opacity-50'}`}>
                          <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${lesson.done ? 'bg-emerald-500/15' : 'bg-[#18181A] border border-[#27272A]'}`}>
                            {lesson.done ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> : <Play className="w-3 h-3 text-[#71717A]" />}
                          </div>
                          <div className="flex-1 min-w-0">
          ...
          <div className="bg-gradient-to-r from-amber-500/10 to-[#F59E0B]/5 border border-amber-500/20 rounded-2xl p-4">
            <div className="flex items-center gap-3">
              ...
              <a href="/admin/assinaturas" className="flex-shrink-0 flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-amber-600 text-white text-xs font-semibold px-4 py-2.5 rounded-xl hover:opacity-90 transition-opacity">
                Gerenciar Planos <ChevronRight className="w-3 h-3" />
              </a>
            </div>
          </div>
