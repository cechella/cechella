...
import {
  ...
  TrendingUp, ChevronRight, ChevronDown, Star, Eye,
} from 'lucide-react'
...
const FALLBACK_MODULES: Module[] = [
  ...
  },
  {
    ...
    lessons: [
      { id: '6', num: 1, title: 'Posicionamento — encontre o nicho que te pagará mais', duration: '1h 02min', video_url: '', is_free: false, done: false },
  ...
  },
  {
    ...
    lessons: [
      ...
      { id: '13', num: 3, title: 'Cultura de alta performance', duration: '1h 03min', video_url: '', is_free: false },
      { id: '14', num: 4, title: 'Liderança situacional', duration: '1h 01min', video_url: '', is_free: false },
    ],
  },
  {
...
export default function AdminEscolaVendasPage() {
  ...
  return (
    <div className="flex h-screen bg-[#0A0A0B] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <TopBar user={{ name: 'Admin', role: 'admin' }} title="Escola de Vendas" />
        <main className="flex-1 overflow-y-auto px-6 py-5">

          {/* Preview badge */}
          <div className="flex items-center gap-2 bg-[#7B3FE4]/10 border border-[#7B3FE4]/25 rounded-xl px-4 py-2.5 mb-5 text-xs text-[#C4B5FD]">
            <Eye className="w-3.5 h-3.5 flex-shrink-0" />
            <span><strong className="text-[9px] font-bold uppercase tracking-wider bg-[#7B3FE4]/25 text-[#A78BFA] px-1.5 py-0.5 rounded mr-2">Preview Admin</strong>
            Esta é a visão exata que o consultor verá. Para gerenciar conteúdo:</span>
            <a href="/admin/treinamento" className="underline text-[#C4B5FD] hover:text-white transition-colors ml-auto flex-shrink-0">Gerenciar Treinamento →</a>
          </div>
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <div className="flex gap-2">
                {[
                  ...
                  { label: 'Aulas', value: allLessons.length },
                  { label: 'Desbloqueados', value: `${modules.filter((m) => m.unlocked).length}/${modules.length}` },
                ].map((s) => (
                  <div key={s.label} className="bg-[#18181A] border border-[#1C1C1E] rounded-xl px-4 py-2.5 text-center min-w-[80px]">
                  ...
                  <div className="p-5">
                    ...
                    <div className="mb-3">
                      <div className="h-1.5 bg-[#1C1C1E] rounded-full overflow-hidden">
                        <div className={`h-full rounded-full bg-gradient-to-r ${mod.color} ${!mod.unlocked ? 'opacity-30' : ''}`} style={{ width: mod.unlocked ? '100%' : '2%' }} />
                      </div>
                    </div>
                  ...
                  {isOpen && (
                    <div className="border-t border-[#1C1C1E]">
                      {mod.lessons.map((lesson) => (
                        <div
                          key={lesson.id}
                          className="flex items-center gap-3 px-5 py-3 border-b border-[#1C1C1E] last:border-0"
                        >
                          <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 bg-[#18181A] border border-[#27272A]">
                            {mod.unlocked
                              ? <Play className="w-3 h-3 text-[#71717A]" />
          ...
          <div className="bg-gradient-to-r from-amber-500/10 to-[#F59E0B]/5 border border-amber-500/20 rounded-2xl p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="font-semibold text-white text-sm">Gerenciar módulos e aulas</p>
                <p className="text-xs text-[#71717A] mt-0.5">Adicione, edite ou remova módulos e aulas para todos os consultores da rede.</p>
              </div>
              <a href="/admin/treinamento" className="flex-shrink-0 flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-amber-600 text-white text-xs font-semibold px-4 py-2.5 rounded-xl hover:opacity-90 transition-opacity">
                Gerenciar Treinamento <ChevronRight className="w-3 h-3" />
              </a>
            </div>
          </div>
