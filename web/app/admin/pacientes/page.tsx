...
const DEMO_PACIENTES: Paciente[] = [
  { id:'1', nome:'Fernanda Oliveira',  telefone:'47991234567', email:'fernanda@email.com',  data_nascimento:'1982-04-15', status:'ativo',    data_inicio:'2024-01-10', proximo_implante:'2026-08-20', numero_implantes:5,  peso:62, altura:165, queixa_principal:'Fadiga e libido baixa',          hormonio_principal:'Testosterona',     created_at:'2024-01-10', updated_at:'2026-07-01' },
  { id:'2', nome:'Mariana Santos',     telefone:'47992345678', email:'mariana@email.com',   data_nascimento:'1978-08-22', status:'ativo',    data_inicio:'2023-06-05', proximo_implante:'2026-09-10', numero_implantes:8,  peso:58, altura:162, queixa_principal:'Fogachos e insônia',             hormonio_principal:'Terapia Combinada',created_at:'2023-06-05', updated_at:'2026-07-15' },
  { id:'3', nome:'Claudia Pereira',    telefone:'47993456789',                              data_nascimento:'1990-02-28', status:'pendente', data_inicio:undefined,    proximo_implante:undefined,    numero_implantes:0,                      queixa_principal:'Humor instável e ganho de peso', hormonio_principal:'Estradiol',        created_at:'2026-07-25', updated_at:'2026-07-25' },
  { id:'4', nome:'Patricia Costa',     telefone:'47994567890', email:'patricia@email.com',  data_nascimento:'1975-11-10', status:'ativo',    data_inicio:'2022-03-15', proximo_implante:'2026-07-31', numero_implantes:12, peso:70, altura:170, queixa_principal:'Perimenopausa',                  hormonio_principal:'Progesterona',     created_at:'2022-03-15', updated_at:'2026-07-20' },
  { id:'5', nome:'Amanda Rodrigues',   telefone:'47995678901',                              data_nascimento:'1988-07-03', status:'inativo',  data_inicio:'2023-09-01', proximo_implante:undefined,    numero_implantes:3,                      queixa_principal:'Ansiedade e irritabilidade',     hormonio_principal:'DHEA',             created_at:'2023-09-01', updated_at:'2026-02-10' },
  { id:'6', nome:'Beatriz Lima',       telefone:'47996789012', email:'beatriz@email.com',   data_nascimento:'1985-12-19', status:'alta',     data_inicio:'2021-01-20', proximo_implante:undefined,    numero_implantes:18, peso:55, altura:158, queixa_principal:'Disfunção sexual feminina',      hormonio_principal:'Dutasterida',      created_at:'2021-01-20', updated_at:'2026-06-01' },
]
...
function avatarColor(nome: string) {
  const colors = ['bg-[#7B3FE4]/20 text-[#9558EE]','bg-blue-500/20 text-blue-400','bg-emerald-500/20 text-emerald-400','bg-pink-500/20 text-pink-400','bg-amber-500/20 text-amber-400','bg-cyan-500/20 text-cyan-400']
  let h = 0; for (let i = 0; i < nome.length; i++) h = (h * 31 + nome.charCodeAt(i)) & 0xffff
...
export default function AdminPacientesPage() {
  ...
  const carregar = useCallback(async () => {
    setLoading(true)
    const { data, error } = await supabase.from('pacientes').select('*').order('updated_at', { ascending: false })
  ...
  const implantesUrgentes = pacientes.filter(p => { const d = diasParaProximoImplante(p.proximo_implante); return d !== null && d <= 7 && d >= 0 }).length
  ...
  async function salvarNovo() {
    const novo: Paciente = {
      id: String(Date.now()), nome: form.nome ?? '', telefone: form.telefone ?? '',
      email: form.email, data_nascimento: form.data_nascimento, status: (form.status as StatusPaciente) ?? 'pendente',
      data_inicio: form.data_inicio, proximo_implante: form.proximo_implante, numero_implantes: form.numero_implantes ?? 0,
      observacoes: form.observacoes, queixa_principal: form.queixa_principal, hormonio_principal: form.hormonio_principal,
      peso: form.peso, altura: form.altura, created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
    }
    await supabase.from('pacientes').insert([novo])
    setPacientes(prev => [novo, ...prev]); setModalNovo(false); setForm({})
  }

  async function excluirPaciente(id: string) {
    await supabase.from('pacientes').delete().eq('id', id)
    setPacientes(prev => prev.filter(p => p.id !== id)); setMenuAberto(null)
  ...
  async function atualizarStatus(id: string, status: StatusPaciente) {
    ...
    setPacientes(prev => prev.map(p => p.id === id ? { ...p, status } : p)); setMenuAberto(null)
  }
  ...
  return (
    <div className="flex h-screen bg-[#0A0A0B] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <TopBar user={{ name: 'Admin', role: 'admin' }} title="Pacientes" />
        <main className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={carregar} className="w-8 h-8 rounded-lg bg-[#111113] border border-[#1C1C1E] flex items-center justify-center text-[#52525B] hover:text-white hover:border-[#3F3F46] transition-all">
                <RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} />
              </button>
              <button onClick={() => { setModalNovo(true); setForm({}) }}
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="relative flex-1 min-w-[200px] max-w-xs">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar paciente, telefone..."
            ...
            {(['todos','ativo','pendente','inativo','alta'] as const).map(s => (
              <button key={s} onClick={() => setFiltroStatus(s)}
                className={`text-xs font-medium px-3 py-2 rounded-lg border transition-all capitalize ${
                  filtroStatus === s ? 'bg-[#7B3FE4]/15 border-[#7B3FE4]/40 text-[#9558EE]' : 'bg-[#111113] border-[#1C1C1E] text-[#71717A] hover:text-white hover:border-[#3F3F46]'
                }`}>
                {s === 'todos' ? 'Todos' : statusConfig[s as StatusPaciente]?.label ?? s}
              </button>
            ))}
            <div className="relative">
              <select value={filtroHormonio} onChange={e => setFiltroHormonio(e.target.value)}
                className="appearance-none bg-[#111113] border border-[#1C1C1E] rounded-lg pl-3 pr-7 py-2 text-xs text-[#71717A] hover:text-white focus:outline-none focus:border-[#7B3FE4]/50 transition-all cursor-pointer">
                <option value="todos">Hormônio: Todos</option>
                {hormonioOptions.filter(h => h !== 'todos').map(h => <option key={h} value={h}>{h}</option>)}
              </select>
          ...
          <div className="bg-[#111113] border border-[#1C1C1E] rounded-2xl overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {filtrados.length === 0 ? (
                    <tr><td colSpan={8} className="text-center py-16 text-[#52525B] text-sm">
                      <Users className="w-8 h-8 mx-auto mb-3 opacity-30" />Nenhum paciente encontrado
                    </td></tr>
                  ) : filtrados.map(p => {
                      ...
                      <tr key={p.id} className="border-b border-[#1C1C1E]/50 hover:bg-[#18181A]/40 transition-colors group">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${av}`}>{getInitials(p.nome)}</div>
                            <div>
                        ...
                        <td className="px-4 py-3"><span className="text-xs text-[#A1A1AA] max-w-[160px] truncate block">{p.queixa_principal ?? '—'}</span></td>
                        <td className="px-4 py-3"><span className="text-xs text-[#52525B]">{formatDate(p.updated_at)}</span></td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1">
                            <button onClick={() => setDetalhe(p)} className="w-7 h-7 rounded-lg bg-[#1C1C1E] hover:bg-[#27272A] flex items-center justify-center text-[#52525B] hover:text-white transition-all opacity-0 group-hover:opacity-100" title="Ver detalhes">
                              <Eye className="w-3.5 h-3.5" />
                            </button>
                            <a href={`https://wa.me/55${p.telefone.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer"
                              className="w-7 h-7 rounded-lg bg-[#1C1C1E] hover:bg-emerald-500/10 flex items-center justify-center text-[#52525B] hover:text-emerald-400 transition-all opacity-0 group-hover:opacity-100" title="WhatsApp">
                              <MessageSquare className="w-3.5 h-3.5" />
                            </a>
                            <div className="relative">
                              <button onClick={() => setMenuAberto(menuAberto === p.id ? null : p.id)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 bg-[#111113] border border-[#1C1C1E] rounded-2xl w-full max-w-lg mx-4 overflow-hidden shadow-2xl shadow-black/50">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#1C1C1E]">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${avatarColor(detalhe.nome)}`}>{getInitials(detalhe.nome)}</div>
                <div>
              ...
              <button onClick={() => setDetalhe(null)} className="w-8 h-8 rounded-lg bg-[#1C1C1E] flex items-center justify-center text-[#52525B] hover:text-white transition-colors"><X className="w-4 h-4" /></button>
            </div>
            ...
            <div className="flex items-center gap-2 px-5 py-4 border-t border-[#1C1C1E]">
              <a href={`https://wa.me/55${detalhe.telefone.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 bg-[#111113] border border-[#1C1C1E] rounded-2xl w-full max-w-lg mx-4 overflow-hidden shadow-2xl shadow-black/50">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#1C1C1E]">
              ...
              <button onClick={() => setModalNovo(false)} className="w-8 h-8 rounded-lg bg-[#1C1C1E] flex items-center justify-center text-[#52525B] hover:text-white transition-colors"><X className="w-4 h-4" /></button>
            </div>
            <div className="p-5 space-y-3 max-h-[60vh] overflow-y-auto">
              ...
              <div>
                ...
                <select value={form.hormonio_principal ?? ''} onChange={e => setForm(prev => ({ ...prev, hormonio_principal: e.target.value }))}
              ...
              <div>
                ...
                <textarea rows={2} placeholder="Descreva a queixa principal..." value={form.queixa_principal ?? ''}
              ...
              <div>
                ...
                <input type="date" value={form.proximo_implante ?? ''} onChange={e => setForm(prev => ({ ...prev, proximo_implante: e.target.value }))}
                  className="w-full bg-[#18181A] border border-[#1C1C1E] rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-[#7B3FE4]/50" />
              </div>
            </div>
            <div className="flex items-center gap-2 px-5 py-4 border-t border-[#1C1C1E]">
              <button onClick={() => setModalNovo(false)} className="flex-1 py-2.5 text-xs font-semibold text-[#71717A] bg-[#1C1C1E] border border-[#27272A] rounded-xl hover:text-white hover:border-[#3F3F46] transition-colors">Cancelar</button>
              <button onClick={salvarNovo} disabled={!form.nome || !form.telefone}
                className="flex-1 py-2.5 text-xs font-semibold text-white bg-[#7B3FE4] rounded-xl hover:bg-[#6D35CC] disabled:opacity-40 disabled:cursor-not-allowed transition-colors">Cadastrar</button>
            </div>
          </div>
        </div>
      )}
