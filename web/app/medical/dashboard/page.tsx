// [SYNC] Gerado automaticamente de admin/dashboard — NÃO editar manualmente
...
export default function AdminDashboard() {
  ...
  async function load() {
    try {
      ...
      if (semContato.length > 0)
        alertas.push({ tipo: 'warn', msg: `${semContato.length} leads sem atualização há +24h`, href: '/medical/crm' })
      const quentes = leads.filter(l => l.temperatura === 'quente' && (l.etapa_agente ?? 0) < 7)
      if (quentes.length > 0)
        alertas.push({ tipo: 'hot', msg: `${quentes.length} leads quentes no funil — prioridade máxima`, href: '/medical/crm' })
      const pendentesRef = referidos.filter(r => r.status === 'aguardando').length
      if (pendentesRef > 0)
        alertas.push({ tipo: 'info', msg: `${pendentesRef} referidos aguardando contato`, href: '/medical/referidos' })
      // Atendimento humano ativo
      const emAtendimentoHumano = leads.filter(l => l.atendimento_humano === true).length
      if (emAtendimentoHumano > 0)
        alertas.push({ tipo: 'human', msg: `${emAtendimentoHumano} lead${emAtendimentoHumano !== 1 ? 's' : ''} em atendimento humano ativo`, href: '/medical/agente' })
      ...
      if (aguardandoContato > 0)
        alertas.push({ tipo: 'info', msg: `${aguardandoContato} lead${aguardandoContato !== 1 ? 's' : ''} aguardando primeiro contato da Ana`, href: '/medical/referidos' })
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('leads-pipeline-medical')
  ...
  return (
    <div className="flex h-screen bg-[#0A0A0B] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <TopBar user={{ name: 'Dr. Ricardo Lima', role: 'doctor' }} title="CEO Overview" />
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="bg-[#111113] border border-[#1C1C1E] rounded-2xl p-5 flex flex-col">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/medical/rede" className="text-xs text-[#06B6D4] hover:text-[#22D3EE] flex items-center gap-1 transition-colors">
