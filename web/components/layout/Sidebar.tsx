...
interface NavSection {
  section?: string   // se definido, mostra label de seção antes
  items: NavItem[]
}

// ─── Flat configs para roles não-admin ────────────────────────────────────────
const flatNav: Record<Exclude<UserRole, 'admin'>, NavItem[]> = {
  patient: [
...
function NavLink({ item, collapsed, pathname }: { item: NavItem; collapsed: boolean; pathname: string }) {
  ...
  return (
    ...
    >
      ...
      {!collapsed && isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-[#7B3FE4]" />}
    </Link>
  )
}
    ...
    <aside className={`relative flex flex-col bg-[#111113] border-r border-[#1C1C1E] h-screen transition-all duration-300 ${collapsed ? 'w-16' : 'w-60'}`}>
      ...
      {!collapsed && (
        <div className="px-3 py-3">
          <div className="rounded-xl px-3 py-2" style={{ background: 'rgba(123,63,228,0.1)', border: '1px solid rgba(123,63,228,0.2)' }}>
